...
function usage() {
  console.log(`
Usage:
  node figma_extract/bin/extract-page-desktop-from-file.mjs <pageName> <rootNodeId> <nodesJsonPath>

Example:
  node figma_extract/bin/extract-page-desktop-from-file.mjs home 13:1321 figma_extract/KEEP_DESKTOP_HOME/home_nodes.json
`);
  process.exit(1);
}
...
const nodesPath = process.argv[4];
if (!pageName || !rootId || !nodesPath) usage();

const outDir = path.join("figma_extract", "pages", pageName);
fs.mkdirSync(outDir, { recursive: true });

const json = JSON.parse(fs.readFileSync(nodesPath, "utf8"));
const root = json?.nodes?.[rootId]?.document;
if (!root) throw new Error(`No document found at nodes['${rootId}'].document in ${nodesPath}`);
...
// 1) copy raw nodes into page folder (so each page folder self-contained)
fs.writeFileSync(path.join(outDir, `${pageName}_nodes.json`), JSON.stringify(json, null, 2), "utf8");

// 2) tree
const lines = [];
walkTree(root, lines, 0);
fs.writeFileSync(path.join(outDir, `${pageName}.tree.desktop.txt`), lines.join("\n"), "utf8");
